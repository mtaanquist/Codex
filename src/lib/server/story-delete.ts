...
// Deletes a story and everything scoped to it, in one transaction, in
// child-first order. Universe-scoped entities (characters, places, lore)
// are not the story's to delete; only the per-story overlays, memberships,
// and story-scoped relationship rows go. Without this a story with any
// content cannot be deleted at all (the FKs are ON DELETE NO ACTION).
export async function deleteStory(db: Database, storyId: string, userId: string): Promise<boolean> {
	...
	await db.transaction(async (tx) => {
		...
		// Only story-scoped relationship rows; universe-wide rows (story_id
		// null) belong to the universe and stay.
