...
A single scene is reviewed right away. A chapter or the whole story is bigger, so the window first shows what the run will send: how many scenes it will read and roughly how many tokens that is. If your endpoint publishes prices for the model you picked for the Reviewer, it also shows roughly what that costs to send; the model's replies cost extra on top. Without a published price you see the token figure alone - no price is guessed. If the figure is above the warning limit on your account page, the window says so and asks you to confirm again. Select Back to change what you are reviewing, or Start review to go ahead.

You can also set a spend cap per review on your account page. A run that reaches it stops after the scene it has just finished rather than in the middle of one, and tells you how far it got and what it spent. Raise the cap or start the same review again to carry on: it skips the scenes it already read. A cap needs a published price to work; if the model has none, the run says the cap could not be applied instead of stopping or ignoring it in silence.
