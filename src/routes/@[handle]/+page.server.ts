...
import { publicAuthorIdentity, publicProfile, publicShelf } from '$lib/server/publish';
...
export const load: PageServerLoad = async ({ params }) => {
	...
	const [shelf, profile, identity] = await Promise.all([
		publicShelf(db, handle),
		publicProfile(db, handle),
		publicAuthorIdentity(db, handle)
	]);
	return { handle, shelf, profile, identity };
};
