...
Four quick-settings controls sit at the top: **entity autocomplete** (off / inline ghost-text / popup menu), **underline known entities** (toggle), **theme** (auto / light / dark), and **focus mode** (toggle). Each has a canonical home in full settings; Session just surfaces the commonly-flipped ones. Changes apply forward, not retroactively — switching autocomplete style mid-sentence affects the next suggestion, not the one currently on screen. Below the quick settings sit the day's word count, the daily goal bar, and the streak card (see Goals under "Beyond the writing surface"; the streak is hideable for anyone who does not want scorekeeping).

The Session tab is for the working preferences above, not the Assistant. The chat surface (rubber duck, world Q&A) lives in its own **Assistant** tab in the right sidebar, a peer of Reference, History, and Session, with the co-author, continuation, and reviewer roles on the editor and review surfaces (see `assistant.md`). The per-story mute lives with that tab.
...
- **Insights.** One page per universe answering "how is it going": progress against each story's target and deadline, activity over time, coverage of the world, and the relationship web. Good means the numbers prompt the next writing session rather than gamify it.
- **Goals.** A per-story target word count and deadline, an account-level daily word goal, and an optional streak, surfaced in the Session tab and on Insights. Deliberately hideable; not everyone wants scorekeeping.
...
**The shelf.** An author can claim a handle and turn on a public profile at `/@handle`. The profile is a simple shelf: an optional short bio, the author's links (http links only, marked nofollow), an optional open-for-commissions line, then the published books as a flat list, newest first, each with its cover. (An earlier sketch grouped the shelf by universe; the flat list is what shipped and reads better for a small shelf.) A reader picks a story and reads it: chapters in order, scenes flowed together in the prose face, with none of the sidebars or worldbuilding around it, and with EPUB and PDF downloads when the edition carries them.
