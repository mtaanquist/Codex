...
19. Outline tree. Drag-and-drop. Optional linking of outline nodes to scenes/chapters. (Shipped as v1.5, then retired in v2.26.0 - migration 0039 drops `outline_nodes`; scene status and the Notes view cover planning instead.)
...
- **Notes tab: ship it or hide it.** SHIPPED (built on develop 2026-06-07, pending release). Built the view: a `notes` table (migration 0048, all four scope FKs modelled), universe- and story-scoped freeform notes with pinned/recent lists, a prose editor with autosave, pin/delete, and full History (preview + restore) reusing the revision machinery. The disabled segment is now a live Notes link in the editor and both plan views. Scene/chapter-attached notes are deferred (columns reserved); notes ride the story, universe, and account exports (under notebook/), with import round-trip deferred.
- **Writing goals and deadlines.** SHIPPED (develop 2026-06-07): an account-level daily word goal (users.preferences) shown with today's progress on the Session tab and Insights (plus days-met-this-month), and per-story optional target words + deadline (migration 0049, additive columns) shown as a progress bar and a due-in/overdue line in the Insights Stories block. Story settings gained a Goals section. Pure helpers (daysMetGoal, daysUntil) unit-tested.
- **TTRPG positioning decision.** DECIDED (2026-06-07): keep TTRPG in the pitch, but define it as campaign prep and worldbuilding (a campaign wiki, the way an Obsidian vault is used), not VTT, maps, encounter math, or stat blocks. The existing primitives carry it: a chapter per session with scenes for beats, entities for the cast and places, story-note overlays for campaign-vs-canon state, and checkpoints to keep a session's plan beside what actually happened. No GM-only/secret-notes or stat-block features are planned (no usage evidence; all feedback to date is novel-shaped). The capability-review gap is closed by documentation, not features: a "running a campaign in Codex" angle folded into the help docs (the Notes article already carries the session-log pattern). Plotlines/arcs and a world-calendar timeline stay parked on their own criteria; TTRPG use is evidence toward the timeline if it ever unparks.
...
- **Write-path rate limits and a shared limiter.** Write limits SHIPPED (develop 2026-06-07): per-user budgets via the existing in-memory limiter - a generous shared autosave bucket across scene/entity/note saves (600/min, well above debounced multi-editor writing) and a tighter upload bucket (60/min), thrown as a 429 from a shared write-guard helper. The single-replica stance is now documented in docs/SELF-HOSTING.md (rate limits are per-process; a shared store is the prerequisite for multiple app replicas). A shared limiter itself stays deferred until the hosted service outgrows one replica.
...
The large, deliberately-deferred work: machine assistance. (Formerly "AI and interop"; the interop half, SillyTavern and lorebook import, was dropped entirely on 2026-06-07 - see "What is not on the roadmap".)
...
- **Quota enforcement and billing.** The plan and entitlement model exists from the start (see `schema.md`); what is deferred is enforcing it (rejecting saves or creates that exceed an entitlement, and maintaining `storage_used_bytes`) and, much further out, charging money for a hosted tier. Billing would add a subscription layer that sets a user's `plan_id`; no payment processing is planned for the foreseeable future.
  - **Note: keep "storage configuration" and "usage & storage" separate.** They are easy to conflate but are two different things. Asset storage _configuration_ already shipped (v2.33.0): the admin panel's "Asset storage" section points uploads at an S3-compatible bucket, and image upload works today. The admin nav's "Usage & storage" item (a `soon` stub) is the deferred half of _this_ quota work: measuring consumption (`storage_used_bytes`), showing per-account usage against entitlements, and enforcing caps. Building "Usage & storage" does not touch the storage config that already exists; it adds accounting and enforcement on top.
...
- SillyTavern and lorebook import (character cards, lorebook JSON, round-trip export). Originally Phase 4 step 24, moved to the candidate pool on 2026-06-04, removed outright on 2026-06-07: the author would not use it and no real audience for it is expected. The `imported_from` column stays as harmless reserved jsonb, and `activation_mode` remains reserved for the Phase 9 LLM context-injection behaviour, which never depended on import.
