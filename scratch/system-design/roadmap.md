...
Codex ships in two shapes from the same codebase: a self-hosted Docker image anyone can run, and a hosted service run by the project maintainer, which is a fleet of those same instances with one per customer. The roadmap targets self-host first because the operator and the first user are the same person; the hosted launch lands once the writing experience is real enough to be worth opening up.
...
- **v2.5** - end of Phase 5. Account lifecycle real (proper sign-up, email verification, password reset, admin approval UI, account self-service). Operational basics in place (off-host backups, transactional email, basic rate-limiting). The hosted fleet can provision a per-customer instance, and authors can share work at a public `/@handle`. Hosted instance can open to invited friends.
- **v2.x and beyond** - anything from Phase 6 candidates that survives contact with real use. AI integration, if and when it happens, lives in this space rather than at a reserved version number. It is explicitly not scheduled.
...
24. SillyTavern character card import (PNG with embedded JSON). Lorebook JSON import. Round-trip export.
25. Assets and images. `assets` table plus a pluggable storage backend (local disk for self-host, S3-compatible for hosted). Upload, drop, and paste images in the editor and insert them as markdown pointing at an app-served path. Story cover image, with a generated default cover for books that have none.
26. Markdown export (zip of files with YAML front matter), now bundling referenced assets into an `assets/` folder and rewriting the links. EPUB export via a Node EPUB library.
...
> **-> v2.0 ships.** History, diffs, and TODO markers are real, imports from SillyTavern and lorebooks work, images and covers are in, exports get your words out of the tool cleanly, and you can publish finished work to a public reading page on your own instance. The v1 feature set is complete for self-hosters, AI excepted.
...
33. Operational essentials for hosting. Off-host database backups (nightly dump to S3-compatible storage, retention policy chosen explicitly). Basic rate-limiting on sign-up and password-reset endpoints. Structured logs to a place you will actually look at. Health-check endpoint for uptime monitoring.
34. Provisioning control plane. The hosted service runs one isolated instance and database per customer (a tenant). A small control plane provisions a new instance, routes its subdomain, issues TLS (Caddy on-demand certificates), and rolls upgrades across the fleet. The application itself stays single-operator and multi-user and knows nothing about the fleet; all of this lives outside it. If per-tenant database overhead bites at small scale, the cheap step is a shared Postgres server with one database per tenant.
35. Central reader site and handle registry. The global `/@handle` namespace lives in the control plane and maps a handle to its tenant. On publish, the worker syncs the frozen edition to a central store; a central reader site renders public pages from that store and never touches a tenant's live database. Self-host already serves these pages from the instance itself, so the code that produces an edition is identical in both places.
...
Driven by real use. Candidates, roughly in order of cost-to-value:
...
- **Guest review (comments, then suggested edits).** An author invites a guest to review one story by magic link; the guest may be an existing user or not, and is never forced to create an account (`review_invitations`, `reviewers`). Threaded, resolvable comments anchored to a scene range come first (`review_threads`, `review_comments`). Word-style suggested edits come second and are the harder half (`review_suggestions`): a guest proposes changes the author accepts or rejects one at a time, never writing to the prose directly, with offsets re-anchored against the current text because review is async. Reuses the diff component from the History work.
...
- **Account security: TOTP and passkeys.** The `user_totp` and `webauthn_credentials` tables are modelled from the start; the enrolment and challenge flows, and the account-settings UI to manage them, are built when account security is worth hardening past passwords.
...
- **Spell-check.** Browser-native first: a `spellcheck` attribute on the editor with `lang` taken from a user language preference (the preferences UI arrives in step 32). A dedicated checker library only if the native one proves insufficient.
...
- **Mobile review UI.** Read-only first; editing much later, if ever.
