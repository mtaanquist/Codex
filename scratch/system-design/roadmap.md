...
34. Provisioning control plane. The hosted service runs one isolated instance and database per customer (a tenant). A small control plane provisions a new instance, routes its subdomain, issues TLS (Caddy on-demand certificates), and rolls upgrades across the fleet. The application itself stays single-operator and multi-user and knows nothing about the fleet; all of this lives outside it. If per-tenant database overhead bites at small scale, the cheap step is a shared Postgres server with one database per tenant.
    ...
    > Scope decision (2026-06-04): for the dozen-writer hosted launch, v2.5 builds a MINIMAL control plane only - a scripted provisioner that brings up an instance, routes its subdomain, and issues on-demand TLS - without fleet-upgrade automation or a polished UI. It lives in its own repo outside Codex. The full fleet-management design above (rolling upgrades, control-plane UI, scale tuning) is deferred to Phase 6, to be built when real multi-tenant demand appears. The single shared instance already covers accounts, approval, and the per-instance public reader, so this is purely about standing up additional tenants by hand when needed.
35. Central reader site and handle registry. The global `/@handle` namespace lives in the control plane and maps a handle to its tenant. On publish, the worker syncs the frozen edition to a central store; a central reader site renders public pages from that store and never touches a tenant's live database. Self-host already serves these pages from the instance itself, so the code that produces an edition is identical in both places.
    ...
    > Scope decision (2026-06-04): deferred to Phase 6 alongside the full control plane. Each instance already serves its own `/@handle` reader pages, which is enough for the dozen-writer launch; a central cross-tenant handle registry and reader site are only needed once there are many tenants sharing one public namespace.
