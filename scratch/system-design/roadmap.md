...
24. Scheduled off-site backups and restore (took over this slot on 2026-06-04 when the SillyTavern import moved to the Phase 6 candidates; pulled forward from step 33 because a tool holding someone's novel is not production ready without disaster recovery). Nightly pg_dump from the worker to any S3-compatible bucket (S3, B2, MinIO, R2), retention pruning, an admin ad-hoc trigger with visible run history, and a documented, drilled restore script. Ships as v1.8.
...
33. Operational essentials for hosting. Basic rate-limiting on sign-up and password-reset endpoints. Structured logs to a place you will actually look at. Health-check endpoint for uptime monitoring. (Off-host backups moved up to step 24 and shipped in v1.8.)
