...
- [~] 32. Account self-service. Split across releases:
  ...
  - [ ] 32-danger. Danger zone, two parts (build export first - it is the "export first" safety net and is read-only):
    - [ ] Export-everything: one archive of the user's universes, stories, scenes, entities, notes, and images as markdown; reuses the step 26 exporter.
    - [ ] Delete-account (design agreed 2026-06-04, GDPR with a regret period): self-service delete re-confirms with the password, then immediately deactivates the account (sign-out + sign-in blocked) and takes down all public editions (prompt public erasure), sets deletion_scheduled_at ~7 days out, and emails a cancellation link that reactivates if clicked in time. A worker purge job (scheduled like backups) runs the full hard cascade after the window: every owned row plus the S3 image objects, then sessions/tokens/user. Admin delete shares the cascade but runs immediately (moderation). Needs an additive deletion_scheduled_at column and a deletion_cancel token kind; the shared purge reuses deleteStory per story.
