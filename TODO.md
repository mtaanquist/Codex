...
- [ ] Design adaptation (author's design in scratch/app-design/new_admin/, handed over 2026-06-04). Redesign /admin and /account onto the new sidebar-shell design system; full scope agreed (avatar, pen name + social links, theme + accent colour, stubbed deferred sections). Build in slices, each its own release toward v2.5:
  - [ ] D1. Design-system port: bring admin.css and pages.css into src/lib/styles, reconcile tokens.css/theme.css deltas (new component classes: .admin-shell/.admin-nav/.admin-card/.admin-table/.field/.seg/.toggle/.tfa-_/.otp-input/.recovery-_ etc.). Foundation for the rest.
  - [ ] D2. /admin shell: sidebar nav + sections - Overview (stat cards + instance health), Users (table with role tags + row actions: approve/reject/publishing/suspend/delete), Backups, Instance (SMTP). Stub AI / Usage / Audit as visible "coming later" nav items.
  - [ ] D3. /account shell + existing fields: sectioned settings (Profile, Security, Display). Move password, sessions, email change, export, delete into Security/Profile; move preferences (autocomplete, scene marks) into Display. Surface handle/bio/profile-public (existing columns).
